...
import { canEditRecruitment } from "@/utils/recruitmentPermissions";
...
export default function RecruitmentEditScreen() {
    ...
    useEffect(() => {
        if (!recruitment || recruitment._id !== recruitmentId) return;
        setCompanyName(recruitment.clientName || "");
        setPosition(recruitment.position || "");
        setRecruiterName(recruitment.recruiterName || "");
        setEmploymentType(recruitment.employmentType || "Full Time");
        setNumberOfOpenings(String(recruitment.numberOfOpenings || 1));
        setClosingDate(recruitment.closingDate || "");
        setDescription(recruitment.description || "");
        setStatus(recruitment.status || "Active");
    ...
    const handleSave = async () => {
        if (isSaving) return;
        if (!canEditRecruitment(role)) {
            Alert.alert(
                "Permission",
                "You do not have permission to edit recruitment campaigns",
            );
            return;
        }
        if (!recruitmentId) return;
        ...
        try {
            ...
            Alert.alert(
                "Error",
                error?.message || "Failed to update recruitment",
            );
        } finally {
            setIsSaving(false);
        }
