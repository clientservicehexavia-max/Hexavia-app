...
>("channels/fetchAll", async (_: void, { rejectWithValue }) => {
    try {
        ...
        const channels = res.data?.channels ?? (res.data as any)?.data ?? [];
...
>("channels/fetchDeleted", async (_: void, { rejectWithValue }) => {
    try {
        const res =
            await api.get<GetDeletedChannelsResponse>("/channel/deleted");
        const channels =
            res.data?.channels ??
...
>("channels/generateCode", async (_: void, { rejectWithValue }) => {
    try {
        const res = await api.get<GenerateCodeResponse>(
            "/channel/generate-code",
        );
...
>("channels/deleteChannel", async (body, { rejectWithValue }) => {
    try {
        const res = await showPromise(
            api.delete<DeleteChannelResponse>("/admin/channels", {
                data: body,
            }),
            "Deleting project...",
            "Project deleted",
...
>("channels/restoreChannel", async (body, { rejectWithValue }) => {
    try {
        const res = await api.post<RestoreChannelResponse>(
            "/channel/restore",
            body,
        );
        return {
            channelId:
                res.data?.channelId ?? res.data?.channel?._id ?? body.channelId,
...
>("channels/deleteTask", async (body, { rejectWithValue }) => {
    try {
        await showPromise(
            api.delete<DeleteTaskResponse>("/channel/delete-task", {
                data: body,
            }),
            "Deleting task…",
            "Task deleted",
...
>("channels/assignMemberTask", async (body, { rejectWithValue }) => {
    try {
        await showPromise(
            api.post<AssignMemberTaskResponse>(
                "/channel/assign-member-task",
                body,
            ),
            "Assigning member…",
            "Member assigned",
